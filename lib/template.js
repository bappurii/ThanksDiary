...
module.exports ={
    template : function (ctg_list, date, button, content) {
        ...
        if (req.path=="/" || req.path=="/create_ctg") {
            button="";
        } else if(req.params.content_id){
            button=`
                <a href="/ctg/${req.params.ctg_id}/create">create</a>
                <a href="${req.path}/amend">amend</a>
                <form action="${req.path}/deleting" method="post">
                    <input type="submit" value="delete">
                </form>
            `;
        } else{
            button=`
                <a href="${req.path}/create">create</a>
            `;
        };
        return button;
    }, 
